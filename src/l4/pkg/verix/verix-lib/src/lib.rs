    ...
    ISR: pc_hal::traits::IrqHandler
{
    ...
    for p in buffer.iter() {
        info!("Sending: {:?}, address: {:p}, 0x{:x}", p, p.addr_virt, p.addr_phys);
    }
    ...
    loop {
        ...
        let mut tx_queue = &mut dev.tx_queues[0];
        ...
        info!("tdbal 0x{:x}, tdbah 0x{:x}, tdlen: {}, tdh: {}, tdt: {}", tdbal, tdbah, tdlen, tdh, tdt);

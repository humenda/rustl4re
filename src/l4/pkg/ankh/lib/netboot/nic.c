...
/* Used by nfs.c */
unsigned char *hostname = (unsigned char*)"";
int hostnamelen = 0;
static uint32_t xid;
static unsigned char *end_of_rfc1533 = NULL;
static const unsigned char broadcast[] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
static const struct in_addr zeroIP = { 0L };
...
static struct in_addr dhcp_server = { 0L };
static struct in_addr dhcp_addr = { 0L };
...
{
	char s[16];
	sprintf(s, "%s", inet_ntoa(client_ip));
#ifndef ANKH
	...
	unsigned short ptype, struct iphdr *ip __unused, struct udphdr *udp __unused)
{
	...
	if (arpreply->opcode != htons(ARP_REPLY)) 
		return 0;
	if (memcmp(arpreply->sipaddr, ptr, sizeof(struct in_addr)) != 0)
		return 0;
...
int ip_transmit(int len, const void *buf)
{
	...
	if (destip == IP_BROADCAST) {
		...
		if (i == ETH_ALEN) {	/* Need to do arp request */
			...
			memcpy(arpreq.sipaddr, &arptable[ARP_CLIENT].ipaddr, sizeof(struct in_addr));
			memset(arpreq.thwaddr, 0, ETH_ALEN);
			memcpy(arpreq.tipaddr, &destip, sizeof(struct in_addr));
		      ...
		      struct iphdr *ip, struct udphdr *udp)
{
	...
	if (memcmp(arpreply->thwaddr, ptr, ETH_ALEN) == 0){
		...
		memcpy(&arptable[ARP_SERVER].ipaddr, arpreply->sipaddr, sizeof(struct in_addr));
		memcpy(&arptable[ARP_CLIENT].ipaddr, arpreply->tipaddr, sizeof(struct in_addr));
		return 1;
	}
	return 0;
}
	...
	struct udphdr *udp)
{
	...
	if (bootpreply->bp_xid != xid)
		return 0;
	if (memcmp((char *)&bootpreply->bp_siaddr, (char *)&zeroIP, sizeof(struct in_addr)) == 0)
		return 0;
	...
	memcpy(rfc1533_venddata, (unsigned char *)(bootpreply->bp_vend), len);
	...
	struct udphdr *udp)
{
	...
	if (dhcpreply->bp_xid != xid)
		return 0;
	if (memcmp((char *)&dhcpreply->bp_siaddr, (char *)&zeroIP, sizeof(struct in_addr)) == 0)
		return 0;
...
int dhcp(void)
{
	...
	for (retry = 0; retry < MAX_BOOTP_RETRIES; ) {
		...
		if (await_reply(await_dhcp, 0, NULL, timeout)) {
			...
			memcpy(&ip.bp.bp_vend[9], &dhcp_server, sizeof(struct in_addr));
			memcpy(&ip.bp.bp_vend[15], &dhcp_addr, sizeof(struct in_addr));
...
#ifdef	MDEBUG
		printf("Received IGMP query for: %s\n", inet_ntoa(igmp->group));
#endif			       
		for(i = 0; i < MAX_IGMP; i++) {
...
#ifdef	MDEBUG
		printf("Received IGMP report for: %s\n", inet_ntoa(igmp->group));
#endif			       
		for(i = 0; i < MAX_IGMP; i++) {
...
int await_reply(reply_t reply, int ival, void *ptr, long timeout)
{
	...
	for (;;) {
		...
		if ((ptype == ARP) &&
			...
			arpreply = (struct arprequest *)&nic.packet[ETH_HLEN];
			memcpy(&tmp, arpreply->tipaddr, sizeof(struct in_addr));
			if ((arpreply->opcode == htons(ARP_REQUEST)) &&
			    (tmp == arptable[ARP_CLIENT].ipaddr.s_addr)) {
				arpreply->opcode = htons(ARP_REPLY);
				memcpy(arpreply->tipaddr, arpreply->sipaddr, sizeof(struct in_addr));
				memcpy(arpreply->thwaddr, arpreply->shwaddr, ETH_ALEN);
				memcpy(arpreply->sipaddr, &arptable[ARP_CLIENT].ipaddr, sizeof(struct in_addr));
...
int decode_rfc1533(unsigned char *p, unsigned int block, unsigned int len, int eof)
{
	...
	while (p < endp) {
		...
		if (c == RFC1533_PAD) {
			...
			memcpy(&netmask, p+2, sizeof(struct in_addr));
		else if (c == RFC1533_GATEWAY) {
			/* This is a little simplistic, but it will
			   usually be sufficient.
			   Take only the first entry */
			if (TAG_LEN(p) >= sizeof(struct in_addr))
				memcpy(&arptable[ARP_GATEWAY].ipaddr, p+2, sizeof(struct in_addr));
		}
			...
			memcpy(&dhcp_server, p+2, sizeof(struct in_addr));
...
void print_network_configuration (void)
{
	...
	if (! eth_probe ())
		...
		printf ("Address: %s\n", inet_ntoa(arptable[ARP_CLIENT].ipaddr));
		struct in_addr i;
		i.s_addr = netmask;
		printf ("Netmask: %s\n", inet_ntoa(i));
		printf ("Server: %s\n",  inet_ntoa(arptable[ARP_SERVER].ipaddr));
		printf ("Gateway: %s\n", inet_ntoa(arptable[ARP_GATEWAY].ipaddr));
	}

...
//    l4_int8_t oldchar = 'a';  // init. to non-ctrl character
    ...
    while(1)
    {
        ...
        case '\a':       // beep
            break;
        case '\r':
            vt100_dec32(&term->returns);  // we consumed one return
            newline = 1;
            buf[i++] = '\n';
            break;
...
void vt100_add_key(termstate_t *term, int code)
{
    ...
    while (*result)
    {
            ...
            (term->term_mode == VT100MODE_COOKED))         // in cooked mode
        {
            ...
            vt100_semaphore_down(&term->keysem);          // we backspaced one char
        }
        else                                          // handle normal chars
        {
            ...
            if (*result == '\n')  // atomically count returns in buffer
                vt100_inc32(&term->returns);
	    ...
	    vt100_semaphore_up(&term->keysem);
        }

...
pub fn main() {
    ...
    let mut query = ClearanceQuery {
        ...
        bearing: Angle::new::<degree>(45.0),
        speed: Velocity::new::<knot>(100.0),
        dur: Time::new::<second>(60.0),
        res: Length::new::<meter>(15.0),
    };
    ...
    loop {
        ...
        unsafe {
            for (i, u) in query.to_u32().iter().enumerate(){
                (*l4::l4_utcb_mr()).mr[i] = *u as l4_umword_t;
            }
        }
        ...
        unsafe {
            ...
            let tag = l4::l4_ipc_call(flta, l4_utcb(), send_tag,
            ...
            match l4::l4_ipc_error(tag, l4_utcb()) {
                0 => { // success
                    let clearance = ClearanceQuery::u32_to_length((*l4::l4_utcb_mr()).mr[0..2].iter().cloned().map(|i| i as u32).collect::<Vec<_>>().try_into().unwrap());
